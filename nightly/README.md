...
`--frontend` takes `pcode`, `jvm`, `dex`, `c`, `lua`, or `jni` (comma-separated,
or repeated) and defaults to all of them. It selects *before* anything runs, so
`--frontend pcode` never invokes the Java toolchain, `--frontend jvm,dex` never
starts Ghidra, and `--frontend c` or `--frontend lua` needs no external toolchain
at all. `jni` is the odd one out: its cases build both halves of a JNI boundary,
so they need the Java toolchain *and* Ghidra. Use it with `--filter` to narrow
further: `--frontend pcode --filter funcptr`.
...
a later regression is caught. Today that list holds pointer arithmetic
(`C:ptrarith`) and `C:defaultmodels` — the latter asserts the shipped *native*
propagation defaults, which `models::defaults_for` hands only to
`VirtualMethodTable::Native` (pcode); a `-l c` import is `Unknown` and gets no
default file, so only the pcode twin can enforce them.
