/* A label on an EMPTY statement (`done: ;`), the body of a `goto` target. This was the
   ...
   lowers to a no-op -- the node shape is identical under tree-sitter-c and
   tree-sitter-cpp, so the shared-core fix closed both mirrors at once. At runtime the
