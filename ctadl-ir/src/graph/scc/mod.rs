...
use crate::index::{idx::Idx, index_vec::IndexVec, slice::IndexSlice};
...
pub trait Annotations<N: Idx> {
    ...
    fn create_annotation(&self, element: N) -> Self::Ann;
...
impl<N: Idx, S: Idx + Ord> Annotations<N> for NoAnnotations<S> {
    ...
    #[inline]
    fn create_annotation(&self, _element: N) -> Self::Ann {}

    #[inline]
    fn annotate_scc(&mut self, _scc: S, _annotation: Self::Ann) {}
}
    ...
    A: Annotations<G::Node>,
{
    ...
    duplicate_set: HashSet<A::SccIdx>,
    ...
    A: Annotations<G::Node>,
{
    ...
    fn start_walk_from(&mut self, node: G::Node) -> WalkReturn<A::SccIdx, A::Ann> {
        self.inspect_node(node)
            .unwrap_or_else(|| self.walk_unvisited_node(node))
    }
    ...
    fn find_state(&mut self, mut node: G::Node) -> NodeState<G::Node, A::SccIdx, A::Ann> {
        ...
        loop {
            ...
            log::trace!(
                "Compressing {node:?} down to {previous_node:?} with state {assigned_state:?}"
            );
    ...
    fn walk_unvisited_node(&mut self, initial: G::Node) -> WalkReturn<A::SccIdx, A::Ann> {
        log::trace!("Walk unvisited node: {initial:?}");
        ...
        let mut stack: Vec<VisitingNodeFrame<G, _, _>> = vec![VisitingNodeFrame {
            ...
            current_component_annotation: self.annotations.create_annotation(initial),
        }];
        ...
        'recurse: while let Some(frame) = stack.last_mut() {
            ...
            let successors = match successors {
                ...
                None => {
                    ...
                    //log::trace!(?depth, ?node);

                    assert!(matches!(self.node_states[node], NodeState::NotVisited));
                ...
                match walk {
                    ...
                    Some(WalkReturn::Cycle {
                        ...
                        if successor_min_depth < *min_depth {
                            //log::trace!(?node, ?successor_min_depth);
                            *min_depth = successor_min_depth;
                    ...
                    Some(WalkReturn::Complete {
                        ...
                        //log::trace!(?node, ?successor_scc_index);
                    ...
                    None => {
                        let depth = depth + 1;
                        log::trace!("Recursing down into {successor_node:?} at depth {depth:?}");
                        //log::trace!(?depth, ?successor_node);
                        ...
                        stack.push(VisitingNodeFrame {
                            ...
                            min_cycle_root: successor_node,
                            successor_node,
                            current_component_annotation: self
                                .annotations
                                .create_annotation(successor_node),
                        });
                        continue 'recurse;
                    }
                }
            ...
            log::trace!(
                "Finished walk from {node:?} with annotation: {current_component_annotation:?}"
            );
            ...
            return_value = Some(if frame.min_depth == depth {
                ...
                log::debug!(
                    "Creating SCC rooted in {node:?} with successor {:?}",
                    frame.successor_node
                );

                let scc_index = self.scc_data.create_scc(deduplicated_successors);

                self.annotations
                    .annotate_scc(scc_index, current_component_annotation);

                self.node_states[node] = NodeState::InCycle {
                    scc_index,
                    annotation: current_component_annotation,
                };
