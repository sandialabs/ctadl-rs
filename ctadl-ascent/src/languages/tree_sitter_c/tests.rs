...
fn array_aggregate_initializer() {
    ...
    // `a[i] = e` builds, so the tainted element reaches the array and `a[0]` carries it to the
    // return. An element is `*(a + i)`: `Offset(i)` on the address then the `deref` naming the
    // memory there, with `Offset(0)` elided -- so element 0 is `a.deref`, element 1 `a.[1].deref`.
    let src = r"
...
fn array_aggregate_initializer_untainted() {
    ...
    let src = r"
        ...
        ";
    let prog = program_from_string(src).0;
    check_assign_or_update(&prog, "a.deref", ["#1"], None);
...
fn nested_array_aggregate_initializer() {
    ...
    // Each level contributes `Offset(i)` + `deref`, with `Offset(0)` elided.
    let src = r"
        ...
        ";
    let prog = program_from_string(src).0;
    // A load/store field is a single symbol, so the two-level write `a[0][0]`
    // (`a.deref.deref`) decomposes through an intermediate load: the outer element is loaded
    // to address the inner one, then the tainted inner element is stored into it.
    check_loads(&prog, "a.deref");
    let dump = format!("{prog}");
    assert!(
        dump.contains(".deref := @p0"),
        "expected the tainted inner element store\n{dump}"
    );
...
#[test_log::test]
fn designated_initializer_writes_the_named_member() {
    // A designated initializer names its destination outright, so `.x = v` writes exactly the
    // `p.x` a later `p.x` read resolves to and the flow is precise.
    let src = r"
            struct P { int x; int y; };
