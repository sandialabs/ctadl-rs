...
//! ASCII block-flow diagram so a function's basic-block control flow can be eyeballed.
//! `dump_ir` is the test-facing entry point; the rest is the rendering pipeline.
