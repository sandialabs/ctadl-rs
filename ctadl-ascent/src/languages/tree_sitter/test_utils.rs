    ...
    I: IntoIterator<Item = &'static str>,
{
    ...
    let expected = if dst_ap.fields.is_empty() {
        ...
        // `f.[4].\[]` is the address `f.[4]` and the field `[]`. A destination with more than
        // one symbol would need intermediate loads and is not a single statement.
...
mod ap_tests {
    ...
    fn subscript_is_symbol_segment() {
        ...
        // `Offset(3), Symbol("[]")` -- see `subscript_access_paths` -- but lua still emits a
