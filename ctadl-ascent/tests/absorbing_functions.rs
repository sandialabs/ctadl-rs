...
use ctadl_ascent::facts::{FunctionId, FlowVariable, Path, TaintState, Label, TaintDirection, PackedInsnSiteId, InsnSiteId, InsnId};
...
fn test_absorbing_functions() {
    ...
    facts.taint.push((main_id, TaintState::Free, FlowVariable::Formal(0i16.into()), Path::empty(), endpoint.clone()));
    facts.taint.push((main_id, TaintState::Free, FlowVariable::Formal(0i16.into()), Path::empty(), endpoint2.clone()));
    ...
    let call_arg = FlowVariable::CallArg { id: packed_site, formal: 0i16.into() };
    
    // Propagate taint from formal to CallArg
    facts.taint.push((main_id, TaintState::Free, call_arg.clone(), Path::empty(), endpoint.clone()));
