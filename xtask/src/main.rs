    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--frontend" => {
                ...
                if names.is_empty() {
                    bail!("--frontend requires at least one of: dex, jvm, pcode, php");
                }
...
Tasks:
  regression                 Run the source-sink taint regression suite.
    --frontend <f>           Only exercise frontend <f>: `pcode`, `jvm`, `dex`, or
                             `php` (default: all). Accepts a comma-separated list
                             ...
                             `--frontend php` needs no toolchain at all.
